...
class updateinfo extends db{

	JPanel p,p1,p2;
	JLabel lblnm;
	JTextField txtid;
	JButton updt,back,srch;
	Statement st;
	ResultSet rs;
	
	JLabel nm,lblidnm,lblid,lblfnm,lblcity,lbladd,lblrgstndt,lblrgdt,lblsal,lblpost,lblcnno,lblname;
	JTextField txtfnm,txtcity,txtsal,txtpost,txtcnno;
	JTextArea txtadd;
	...
	updateinfo()
	{
		...
		updt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String fnm,cty,cn,rgdt,sal,po,ad;
				
				fnm = txtfnm.getText();
				cty = txtcity.getText();
				cn = txtcnno.getText();
				rgdt = lblrgdt.getText();
				sal = txtsal.getText();
				po = txtpost.getText();
				ad = txtadd.getText();
				...
				try
				{
				PreparedStatement myst = con.prepareStatement("update emp_info set fname=?,city=?,contactno=?,salary=?,post=?,address=? where id="+id);
				
				myst.setString(1, fnm);
				myst.setString(2, cty);
				myst.setString(3, cn);
				myst.setString(4,sal);
				myst.setString(5,po);
				myst.setString(6,ad);
